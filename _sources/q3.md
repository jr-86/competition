...
The number of ways of closing the lines will be one less than the number of subgraphs of $K_6$ that connect all nodes, as one subgraph is $K_6$ itself, which doesn't involve closing any railway lines.
...
Thus the total number of subgraphs to consider is $2^{15} = 36728$.

These can be grouped by the number of edges they contain, the number of each given by $\binom{15}{e}$ for a graph of $e$ edges.

We want to complete the following table with numbers of connected and disconnected graphs for each number of edges, and total the connected subgraphs.
...
For $e = 5$, each connected subgraph must be a tree, there aren't enough edges to include cycles. The number of spanning trees on a complete graph of $n$ vertices is given by Cayley's Formula $n^{n-2}$. $6^4 = 1296$ subgraphs are connected.
...
- Loop through each subgraph, noting its size and using an algorithm to determine which nodes it connects
- Print the results
